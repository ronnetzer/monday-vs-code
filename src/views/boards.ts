        ...
        if (element) {
            ...
            if (element instanceof BoardTreeItem) {
                ...
                return this.boardsManager
                    .getBoardGroups(element.board.id)
                    .then((groups) =>
                        groups.map(
                            (group) =>
                                new GroupTreeItem(
                                    group,
                                    element,
                                    group.items.length ? vscode.TreeItemCollapsibleState.Collapsed : undefined,
                                ),
                        ),
                    );
            } else {
                // element is GroupItem
                return Promise.resolve(element.group.items).then((items) =>
                    items.map((item) => new ItemTreeItem(item, element)),
                );
            }
        } else {
